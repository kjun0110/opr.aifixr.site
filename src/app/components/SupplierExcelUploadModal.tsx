...
interface SupplierExcelUploadModalProps {
  onClose: () => void;
  supplyChainInfo: {
    scnCode: string;
    productName: string;
...
export default function SupplierExcelUploadModal({ onClose, supplyChainInfo }: SupplierExcelUploadModalProps) {
      ...
      >
        ...
        <div className="p-6">
          ...
          <div className="mb-6 p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl border border-blue-200">
            <div className="font-semibold text-gray-900 mb-3">업로드 대상 정보</div>
            <div className="grid grid-cols-5 gap-4">
              <div>
                <div className="text-xs text-gray-600 mb-1">공급망 ID</div>
                <div className="font-semibold text-sm text-[#00B4FF]">{supplyChainInfo.scnCode}</div>
              </div>
              <div>
                <div className="text-xs text-gray-600 mb-1">제품명</div>
                <div className="font-semibold text-sm">{supplyChainInfo.productName}</div>
              </div>
              <div>
                <div className="text-xs text-gray-600 mb-1">협력사명</div>
                <div className="font-semibold text-sm">{supplyChainInfo.supplierName}</div>
              </div>
              <div>
                <div className="text-xs text-gray-600 mb-1">Tier</div>
                <div className="font-semibold text-sm">
                  <span className="px-2 py-1 bg-[#00B4FF] text-white rounded text-xs">
                    {supplyChainInfo.tier}
                  </span>
                </div>
              </div>
              <div>
                <div className="text-xs text-gray-600 mb-1">기간</div>
                <div className="font-semibold text-sm">{supplyChainInfo.period}</div>
              </div>
            </div>
          </div>
              ...
              <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-xl">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <button 
                      onClick={() => toast.success('협력사 데이터 템플릿을 다운로드합니다')}
